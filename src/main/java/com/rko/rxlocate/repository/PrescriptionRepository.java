...
public interface PrescriptionRepository extends JpaRepository<Prescription, Long> {
    ...
    @Query("SELECT new com.rko.rxlocate.dto.DistrictDTO(dis.id, d.name, dis.name, dr.name, COUNT(p), dis.lat, dis.lng) " +
            "FROM Prescription p " +
            "JOIN p.area ar " +
            "JOIN ar.district dis " +
            "JOIN dis.division d " +
    ...
    @Query("SELECT new com.rko.rxlocate.dto.AreaDTO(ar.id, dis.name, ar.name, dr.name, COUNT(p), ar.lat, ar.lng) " +
            "FROM Prescription p " +
            "JOIN p.area ar " +
            "JOIN ar.district dis " +
            "JOIN dis.division d " +
